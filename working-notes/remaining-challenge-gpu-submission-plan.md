...
The 2026-06-13 solution audit initially reduced the broad not-submit-ready bucket to 68 explicitly deferred solutions. Follow-up baseline work upgraded 67 of those solutions; a later production baseline pass then found eight piped-stdio baselines needing official-protocol-safe replay evidence, and `substring-ab-program-frontier-cs-algorithmic-23` was explicitly deferred for official-scale checker work. Corrected 2026-06-14 official-private replay proved those eight piped baselines are protocol-safe. Follow-up work on 2026-06-24 cleared the remaining two hard algorithmic deferrals, so the default submitter now has no challenge-name deferrals.
...
The challenge-name deferred set is documented in `working-notes/challenge-solution-baseline-audit.md` and mirrored in the `agentics-submit-baselines` default defer list. As of 2026-06-24, that list is empty. `colored-ball-pole-sorting-frontier-cs-algorithmic-142` has a divide-and-conquer construction under the 2,000,000-operation cap, and `substring-ab-program-frontier-cs-algorithmic-23` has official-scale trusted-checker budget for the exhaustive cases.
...
- `agentics-submit-baselines` now provides the resumable production baseline submitter. It reads the normal local agent token source, skips known deferred solutions by default, requires at least one explicit `--target`, submits one challenge-target pair at a time, waits to terminal state, records JSONL progress, and sleeps five seconds before continuing.
...
- A focused metadata review cleaned stale wording for official-capable baselines and expanded the default defer list to the 68 initially deferred solutions. A follow-up subagent pass upgraded 67 of those solutions and initially narrowed the default defer list to `colored-ball-pole-sorting-frontier-cs-algorithmic-142`. A later official baseline pass temporarily re-expanded the default defer list to 10 solutions. Corrected official-private replay removed the eight piped-stdio entries from that default challenge-name defer list, and the 2026-06-24 hard-case pass removed `colored-ball-pole-sorting-frontier-cs-algorithmic-142` and `substring-ab-program-frontier-cs-algorithmic-23`.
- Broad CPU baseline submission can now run through `just prod::submit-baselines --target linux-arm64-cpu`. Broad GPU baseline submission can use the same submitter with an explicit CUDA target such as `--target linux-arm64-cuda` once the operator wants to spend the GPU time.
...
- Inputs should include API base URL, challenge repo path, optional allowlist file, one or more mandatory explicit target filters, delay seconds defaulting to 5, and a dry-run mode. Broad runs without target flags must fail closed.
