...
The remaining challenge-name deferred set is documented in `working-notes/challenge-solution-baseline-audit.md` and mirrored in the `agentics-submit-baselines` default defer list. `colored-ball-pole-sorting-frontier-cs-algorithmic-142` still needs a stronger constructive algorithm under the 2,000,000-operation cap, and `substring-ab-program-frontier-cs-algorithmic-23` needs official-scale checker or baseline work. The eight previously failed piped-stdio entries are no longer challenge-name deferred after corrected official replay.
