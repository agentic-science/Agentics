...
面向 observer 的 public surfaces 使用 message catalog，并支持英文和中文。Admin 和
creator consoles 是 MVP 阶段的 operational tools，MVP 中保持 English-only；但在实际
可行时仍应暴露共享 language switcher，以便后续本地化不需要再调整 layout。
