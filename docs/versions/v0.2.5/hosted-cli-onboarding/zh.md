...
轮询直到 status 为 `completed` 或 `failed`。`status` 必须为 official submission
传入 `--kind solution-submission`，为 `validate --remote --no-wait` 返回的
validation run ids 传入 `--kind validation-run`。完成的 official submission 应包含
