...
- 执行 strict DGX profile preflight，包括 Docker writable-layer quota 和 phase
  mount canary probes。
...
| Strict DGX profile | 以 `agentics` 用户通过；NVIDIA runtime 可见；Docker quota probe 按预期因 quota exhaustion 失败 |
