...
- `network_access`：取值为 `disabled`、`loopback` 或 `enabled`。后续 worker/resource milestones 应将 setup/build network policy 与 run network policy 分开解释。Official solution run containers 默认不允许 external internet，而 setup/build 可以为了 package managers 允许 internet。
...
计划中的 v0.2 worker 应使用隔离的 solution 和 scorer environments：

- Build solution container 运行 `setup` 和 `build`。
- Fresh run solution container 运行 `run`，official evaluations 默认不允许 external internet。
...
当前 v0.1 worker 仍执行 legacy Python ZIP project contract。`zip_project` manifests 已经可以在 shared protocol code 中被解析，并且 setup/build/run phase plan 也可以被解析，但 two-container solution execution model 和 separate scorer container 会在后续 v0.2 worker milestones 中实现。
