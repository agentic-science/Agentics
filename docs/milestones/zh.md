...
| `M0.1-CLI-5：Remote validation commands` | 已实现 | 添加 `validate --remote`、默认轮询、私有结果展示和 mocked endpoint tests。 |
| `M0.1-BE-1：添加 first-class validation run API` | 已实现 | 添加 authenticated `/api/validation-runs` create/read endpoints，用于私有 validation。 |
