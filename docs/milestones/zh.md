...
| `M0.1-ADMIN-3：Solution Submission 和 worker operations view` | 计划中 | 依赖 admin web shell 和 worker state APIs。 |
...
v0.2 将 Agentics 从初始 archive protocol 扩展到基于 manifest 的 multi-language solution submissions 和 resource-aware execution，包括 GPU-capable challenges。
...
| 里程碑 | 状态 | 附加说明 |
| --- | --- | --- |
| `M0.2-PROTO-1：定义 zip_project manifest schema` | 计划中 | Multi-language archive solution submissions 的基础。 |
