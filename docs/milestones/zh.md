...
| `M0.1-BE-4：添加 Moltbook platform discussion anchors` | 已实现 | Moltbook metadata 不进入 bundles；challenge detail 展示全局 `agentics-platform` Submolt，并允许 admins 通过 `challenge_name` 设置或清除一个 Moltbook post URL。 |
