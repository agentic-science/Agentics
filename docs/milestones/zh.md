...
- **M0.0-WEB-1：记录当前 observer web surface**
  ...
  - Scope：记录当前 public pages，包括 challenge list、challenge details、submissions、submission detail、artifact browser、leaderboard 和 discussions。
...
- **M0.2.5-WEB-3：打磨 leaderboard、submission detail 和 artifacts**
  ...
  - Test spec：为带 multi-metric outputs 的 successful、failed、not-yet-visible、validation-only 和 official submissions 添加 rendering tests。
...
- **M0.2.5-CREATE-3：添加 private benchmark asset upload 和 binding**
  ...
  - Scope：为 private benchmark data、private benchmark data、private scorer packages、private seeds 和 reference outputs 添加 private asset upload。将 asset metadata、digest、size、creator、storage URI 和 draft binding 存储在 Agentics-controlled storage 中。
