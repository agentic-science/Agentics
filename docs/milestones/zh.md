...
- Scope：实现尊重 `.gitignore` 的 ZIP packaging、archive validation、`agentics submit <challenge-id> --round <round-id> --target <target-id>`、`agentics submissions show|wait|logs|rank` 和 result display。
...
- **M0.1-CLI-5：Remote validation commands**
  ...
  - Scope：实现 `agentics validate --remote <challenge-id> --round <round-id> --target <target-id>`、validation status polling 和 validation result display，且不更新 leaderboard。
...
- **M0.1-BE-1：添加 first-class validation run API**
  ...
  - Scope：添加 authenticated endpoints，用于创建 validation runs、轮询 validation status、读取 validation results，并在所选 round 和 benchmark target 关闭 validation 时拒绝 validation requests。
...
- **M0.2-TARGET-1：定义 benchmark target schema**
  ...
  - Scope：将单个 challenge-version resource profile 的假设替换为一个或多个 benchmark targets。MVP 支持 Docker platform `linux/arm64` 上的 `linux-arm64-cpu` 和 `linux-arm64-cuda`；`linux-amd64-cpu` 和 `linux-amd64-cuda` 保留给 post-MVP deployment expansion。每个 target 拥有 image references 或 digests、resource limits、validation availability、quota scope 和 ranking scope。
  - Test spec：为 ARM64 CPU target、ARM64 CUDA target、AMD64 target rejection、重复 target ids、不支持的 Docker platforms、缺失 target references、target-specific validation disabled、CUDA hardware metadata，以及无效 image 或 resource metadata 添加 schema 和 bundle validation tests。
...
- **M0.2-DOC-3：记录 benchmark target authoring**
  ...
  - Scope：记录 CPU target ids、Docker platform selection、单 target 与双 target challenges、target-specific validation availability、round-and-target-specific leaderboard behavior、all-target submission semantics，以及未来 GPU targets 如何扩展同一模型。
...
- **M0.2.5-CREATE-1：定义 public challenge manifest 和 repository layout**
  ...
  - Scope：定义 `agentics.challenge.json`、public repository directory layout、lifecycle metadata、archive metadata、namespace rules、required bundle rounds 和 CI validation expectations。
...
- **M0.2.5-CREATE-5：添加 challenge archive flow 并拒绝 version updates**
  ...
  - Scope：拒绝 `new_version` drafts，因为实质 benchmark 变更必须使用新的 challenge id。添加 challenge archive drafts，保留 public records、保留 private assets、从默认浏览隐藏 challenges，并禁用新的 validation 或 official runs。
