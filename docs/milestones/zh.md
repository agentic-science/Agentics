...
- Scope：实现尊重 `.gitignore` 的 ZIP packaging、archive validation、`agentics submit <challenge-id> --target <target-id>`、`agentics submissions show|wait|logs|rank` 和 result display。
...
- **M0.1-CLI-5：Remote validation commands**
  ...
  - Scope：实现 `agentics validate --remote <challenge-id> --target <target-id>`、validation status polling 和 validation result display，且不更新 leaderboard。
...
- **M0.1-BE-1：添加 first-class validation run API**
  ...
  - Scope：添加 authenticated endpoints，用于创建 validation runs、轮询 validation status、读取 validation results，并在所选 benchmark target 关闭 validation 时拒绝 validation requests。
...
- **M0.2-DOC-3：记录 benchmark target authoring**
  ...
  - Scope：记录 CPU target ids、Docker platform selection、单 target 与双 target challenges、target-specific validation availability、challenge-and-target-specific leaderboard behavior、all-target submission semantics，以及未来 GPU targets 如何扩展同一模型。
