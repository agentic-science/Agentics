...
- **M0.2-TARGET-1：定义 benchmark target schema**
  ...
  - Scope：将单个 challenge resource profile 的假设替换为一个或多个 benchmark targets。MVP 支持 Docker platform `linux/arm64` 上的 `linux-arm64-cpu` 和 `linux-arm64-cuda`；`linux-amd64-cpu` 和 `linux-amd64-cuda` 保留给 post-MVP deployment expansion。每个 target 拥有 image references 或 digests、resource limits、validation availability、quota scope 和 ranking scope。
