...
- **M0.1-ADMIN-1：Admin web shell 和 authentication**
  ...
  - Scope：添加 admin routes、面向 web console 的 GitHub OAuth human sessions、面向自动化的 admin service-token integration、layout、navigation 和 access-denied handling。
...
| `M0.1-ADMIN-1：Admin web shell 和 authentication` | 已实现 | 新增符合 VIS 的 `/admin` route group、面向 web console 的 GitHub OAuth human sessions、面向服务器侧工具的 admin service tokens，以及 admin API client。 |
...
- **M0.2.5-CLI-2：添加 challenge review record reviewer commands**
  ...
  - Scope：添加使用 admin service tokens 的 admin validation、approval、rejection、publish、abandon 和 cleanup CLI helpers。Creator-side review record creation、review record status 和 private asset upload 在 CLI 支持 GitHub OAuth creator sessions 之前保持 web-only。
