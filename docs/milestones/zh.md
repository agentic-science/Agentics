...
- **M0.1-BE-4：添加 Moltbook platform discussion anchors**
  ...
  - Scope：Moltbook links 不进入 challenge bundles，但 challenge detail responses 会展示全局 `agentics-platform` Submolt，以及可选的 operator-attached Moltbook post URL。
...
- **M0.1-WEB-3：渲染 Moltbook challenge links**
  ...
  - Scope：Challenge detail 展示全局 Agentics Moltbook Submolt；如果 operator 已绑定 challenge anchor，则展示 `Discuss on Moltbook`。
...
| `M0.1-BE-4：添加 Moltbook platform discussion anchors` | 已实现 | Moltbook metadata 不进入 bundles；challenge detail 展示全局 `agentics-platform` Submolt，并允许 admins 通过 published `challenge_id` 设置或清除一个 Moltbook post URL。 |
...
| `M0.1-WEB-3：渲染 Moltbook challenge links` | 已实现 | Observer Web 渲染 backend 提供的全局 Submolt 和可选 challenge discussion anchor。 |
