...
- **M0.1-CLI-3：Solution workspace initialization**
  ...
  - Scope：历史 v0.1 bootstrap for `agentics init-solution <challenge-id>`；当前实现已被 M0.2 manifest-based `zip_project` workspace generator 取代。
...
- **M0.1-CLI-4：Solution Submission packaging 和 official submit**
  ...
  - Scope：实现尊重 `.gitignore` 的 ZIP packaging、archive validation、`agentics submit <challenge-id> --target <target>`、`agentics submissions show|status|wait|logs|rank` 和 result display。
...
- **M0.1-CLI-5：Remote validation commands**
  ...
  - Scope：实现 `agentics validate --remote --challenge-id <challenge-id> --target <target>`、validation status polling 和 validation result display，且不更新 leaderboard。
...
- **M0.1-WORKER-2：持久化 aggregate 和 per-run metrics**
  ...
  - Scope：存储 declared aggregate metrics、optional per-run metrics、rank score、ranking metadata 和 evaluator diagnostics。
  - Test spec：为 valid metrics、missing rank score、non-finite values、unknown metrics 和 per-run payloads 添加 evaluator-output fixture tests。
...
| `M0.1-BE-4：添加 Moltbook platform discussion anchors` | 已实现 | Moltbook metadata 不进入 bundles；challenge detail 展示全局 `agentics-platform` Submolt，challenge list 展示可选 anchor，并允许 admins 通过 published `challenge_id` 设置或清除一个 Moltbook post URL。 |
...
| `M0.2-CLI-3：选择 targets` | 已实现 | `submit <challenge-id>` 和 `validate --remote --challenge-id <challenge-id>` 支持 `--target` 和 `--all-targets`；CLI preflight 会在 packaging 前拒绝 unsupported targets 和 target-disabled validation。 |
...
- **M0.2.5-DGX-2：添加 DGX Spark deployment profile**
  ...
  - Scope：定义 DGX-specific environment values、persistent storage layout、reverse proxy 和 TLS assumptions、Docker settings、service supervision、backup locations 和 release artifact paths。包括一个由 loopback XFS data-root image 和 project quotas 支撑的 Agentics-owned Docker daemon、`AGENTICS_RUNNER_SECURITY_PROFILE=production`、`AGENTICS_HOST_PROBE_MODE=require`、`AGENTICS_WORKER_ACCELERATORS=gpu`、`AGENTICS_WORKER_GPU_PROBE_IMAGE`、Docker writable-layer quota probes，以及位于 per-phase loopback filesystem images 下、由 root 预先准备的 XFS project-quota slots，用于所有 solution setup/build/run writable mounts 和 evaluator prepare/score writable mounts。
...
| `M0.2.5-DEPLOY-1：添加 hosted deployment baseline` | 已实现 | 已文档化 Local Compose MVP deployment rehearsal；DGX Spark hosted profile 现在由 DGX-1 和 DGX-2 单独覆盖。 |
...
- **M0.3-ADMIN-1：添加 PR review 和 official-run controls**
...
| `M0.3-ADMIN-1：添加 PR review 和 official-run controls` | 计划中 | PR workflow 的 admin control。 |
