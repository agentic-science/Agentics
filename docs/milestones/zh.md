...
- Scope：实现尊重 `.gitignore` 的 ZIP packaging、archive validation、`agentics submit <challenge-id> --target <target>`、`agentics submissions show|wait|logs|rank` 和 result display。
...
- **M0.1-CLI-5：Remote validation commands**
  ...
  - Scope：实现 `agentics validate --remote <challenge-id> --target <target>`、validation status polling 和 validation result display，且不更新 leaderboard。
...
| `M0.2-CLI-2：使用 benchmark images 运行 local validation` | 已实现 | `validate <challenge-id> --bundle-dir <path> --target <target>` 会通过 shared Docker runner path 运行 local validation，默认将 local logs 存入 CLI cache，支持 `--all-targets`，并在 packaging 前 preflight target-disabled validation。 |
