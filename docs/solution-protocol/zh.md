...
`../../docker/images/cpu-base/README.md`。在该 image 发布并 pinned by digest
...
当前实现已经将 benchmark target 作为 challenge version 的 first-class execution 和 ranking scope。
...
AMD64 Linux targets 保留给 post-MVP deployment expansion。一个 challenge
version 可以选择一个或多个 deployment-supported targets。Validation runs、official
evaluations、capacity accounting 和 leaderboards 都会按 target 隔离。一个
solution submission 可以请求一个 target；CLI 的 `--all-targets` option 会为每个
supported target 创建一次 evaluation。
