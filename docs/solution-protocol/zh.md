...
当前实现会在 submission 阶段校验 ZIP project manifest，在 Docker 中执行 setup/build/run phases，在单独的 Docker container 中运行 challenge-owned evaluator，并强制执行 challenge-declared resource profiles。DGX-first MVP targets 的 target-specific platform selection 已实现。CLI 可以基于 checked-out challenge bundle 中的 public validation data 运行 local benchmark-image validation。Heterogeneous GPU scheduling 和 GPU quota enforcement 仍属于独立 milestones。
...
setup/build workspace scratch、run `/io`、prepare `/prepared`、evaluator
...
对于 `coexecuted_benchmark`，bundle 声明 `execution.benchmark.command`、
`execution.benchmark.result_file` 和 `execution.acknowledge_danger: true`。它可以声明
`execution.validation_prepare` 和 `execution.official_prepare`；这些 prepare specs 只包含
...
evaluator image 中运行一次可信 benchmark harness。Benchmark 会接收 `/workspace`、
`/challenge`、可选 `/prepared` 和 writable `/output`；它负责决定如何从 `/workspace`
...
benchmark harness、participant-built workspace 和 private official benchmark files 会共享同一个
evaluator-image container。Challenge owners 不得把 secrets 放入 co-executed environments；
...
official 使用 private runtime bundle；`/prepared` 作为可写 prepared-data directory、
...
对于 `coexecuted_benchmark`，prepare specs 只包含 `command` 和可选
`reproducibility_notes`。Prepare command 会收到 `--challenge-dir`、`--prepared-dir`、
