...
keys。Validation jobs 使用 public-only bundle key，official jobs 使用 private runtime
bundle key。如果 publish 失败，cleanup 会删除 temporary work directories 以及该 publish
...
Published runtime bundles 会打包成 immutable archives 并存入 durable object
