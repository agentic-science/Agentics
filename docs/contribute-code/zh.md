...
`agentics-rust-toolchain:trixie-llvm22-local` image。该 image 由
...
Dev 和 test Compose 使用 `postgres:18-alpine`，并按 PostgreSQL 18 官方 image
要求把 data volume 挂载到 `/var/lib/postgresql`。它们以
`io_method=io_uring` 运行 Postgres，并只对 Postgres service 设置
`seccomp=unconfined`；这是当前 Linux Docker host 上启动 PG 18 `io_uring` 所必需的。
...
这两个文件都不应提交。
Local HTTP loopback sign-in 还要求
`AGENTICS_WEB_SESSION_COOKIE_SECURE=false`，这是已提交的 dev 默认值。Non-loopback
GitHub App redirect URLs 必须使用 HTTPS。
...
Dev launcher 在启动前也会检查 API 和 web 的 host-published ports，并拒绝使用
production 或 rehearsal ports，因此 local development 可以在 production 保持运行时启动。
...
只有在需要 production frontend build artifact，或改动 build-time behavior 时，才运行
`cd frontends/web && bun run build`。
