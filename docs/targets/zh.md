...
Score distributions 只会按 visibility policy 暴露声明过的 challenge metrics。只有当
challenge 的 primary metric 在 metric schema 中声明为 `visibility: "public"` 时，该
primary metric 才能出现在 public score distributions 中。即使 leaderboard 本身是
public，official-only primary metrics 仍会在 public distributions 中 redacted。Public
solution 和 leaderboard DTOs 会把 completed official primary metric 暴露为
`official_primary_metric: { metric_name, value }`，而不是匿名 score，因此 clients
可以把它标注为 `<metric label> (Primary)`。
