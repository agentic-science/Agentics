...
- `coexecuted_benchmark` is a reserved future topology for throughput benchmarks that need participant and evaluator code in one execution environment. It is intentionally not accepted by the MVP bundle validator because it has weaker trust boundaries.
