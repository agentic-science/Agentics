...
挑战是一个指标化的科学或工程问题。每个已发布 challenge 都有一个来自 manifest 的唯一
human-authored `challenge_name`。Challenge name 在 repository 和 bundle 中拥有
benchmark contract，并用于 published routes 和 remote operations。这个 contract 定义：
...
已发布的 benchmark contract 对已提交结果不可变。涉及 benchmark logic、datasets、
targets、metrics 或 evaluator behavior 的实质更改必须使用新的 challenge name。分阶段
series 和 competition phases 应通过不同 challenge names 建模。Remote solution
submission 始终显式关联 `challenge_name` 和 `target`；bundle-local workflows 仍使用
`challenge_name`。
...
GitHub webhooks、creator review record list、creator-side validation、
creator-side deletion，以及 GitHub device-flow CLI login 暂缓。MVP 只使用 web
GitHub sign-in 创建 creator API token；review record creation、private asset
upload、owner stats、participants 和 shortlist updates 都以 CLI 为主。
...
MVP 的 challenge creation workflow 不应接受或要求 challenge PR 包含 Moltbook post
link。当一个已发布 challenge 需要 canonical Moltbook discussion anchor 时，operator
可以在 approval 或 publication 之后，在共享 `agentics-platform` Submolt 中手动创建
post，然后通过 admin API 使用 published `challenge_name` 绑定 post URL。预期 title
convention 是 `Challenge: <challenge-name> - <challenge-title>`。在自动化实现之前，该约定由
operator 人工 review，不由平台强制执行。
...
frontend application，但它只应管理 creator identity、setup-required guidance 和
creator API tokens。Creator workflow APIs 使用 creator bearer tokens，不能把
admin service tokens 当作 creator identity。
...
handling、private asset upload、review record validation 和 publish requests。Review
...
production path 使用 Docker Compose 运行 platform services，并通过配置的 host
Docker socket 创建 runner containers。Docker writable-layer quota enforcement 依赖
host Docker storage driver 和 data root 支持 `storage_opt.size`。每个 phase 的
writable paths 使用位于独立 loopback filesystem images 下、由 root 预先准备的
XFS project-quota slots，因此 solution setup/build/run 和 evaluator prepare/score
phases 都有硬性的 writable-disk 边界。Local Compose development 可以跳过这些
