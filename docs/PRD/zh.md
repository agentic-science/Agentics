...
- Challenge-owned prepare phases 可以在 solution invocations 之前用 evaluator image 运行，在 prepared workspace 下生成 official inputs、reference outputs 和 run manifest。
...
- `piped_stdio` 支持一个 interactive session，其中 trusted interactor/evaluator 通过有界 stdin/stdout pipes 与一个 participant run container 并发运行。Private session data 只挂载到 interactor 侧，interactor 仍然写出标准 evaluator result JSON。
- `coexecuted_benchmark` 支持需要把 participant-built workspace files 和可信 benchmark harness 放在同一个 evaluator-image container 中的 throughput-style benchmarks。它必须显式声明 challenge-owner `acknowledge_danger: true`，禁止 `resource_profile.solution.run`，并采用更弱的 trust boundary：official private benchmark files 会与 participant code 共享同一个 container。
...
- Docker 运行 benchmark/evaluator containers。
