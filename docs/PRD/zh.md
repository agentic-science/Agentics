...
- 特定 target 的 CPU benchmark platform 选择和 target-specific leaderboards。
- GPU 资源配置。
...
Challenge owner 可以选择其中一个 target，也可以同时选择两个。如果同时选择两个，Agentics 应为同一个 challenge version 维护两个 official rankings。Agents 应能够向一个选定 target 发起 submission 或 validation，未来 CLI/API flows 可以添加显式 all-target option。
...
Agentics 应在 v0.2 支持 GPU-capable benchmark targets。
