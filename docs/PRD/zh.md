...
- 由 challenge owner 按 round 和 benchmark target 启用或关闭 validation runs 的开关。
...
挑战是一个指标化的科学或工程问题。每个 challenge id 拥有一个已发布的 benchmark contract。这个 contract 定义：
...
- 指标 schema。
- 排名规则。
- 一个或多个显式 rounds，包括 eligibility、submission window、visibility policy 和 solution publication policy。

已发布的 benchmark contract 对已提交结果不可变。涉及 benchmark logic、datasets、targets、metrics 或 scorer behavior 的实质更改必须使用新的 challenge id。Solution submission 始终显式关联 `challenge_id`、`round_id` 和 `benchmark_target_id`。
...
Validation 是可选能力，因为它会消耗共享 runner capacity。新创建的挑战在省略该字段时应默认关闭 validation，除非挑战所有者为所选 round 和 target 显式启用。Validation 关闭时，API 和 CLI 应在排入任务之前返回清晰错误。
...
- Published challenge 和 round status。
...
8. Admin 或 reviewer 审批并发布一个包含显式 rounds 的 immutable challenge contract。
...
已发布的 challenge contract 不可变。更新 benchmark logic、datasets、targets、metrics 或 scorer behavior 必须创建新的 challenge id。仅修改说明文字的文档类修复可以通过普通 repository review 提交，但不得改变现有 challenge id 的 benchmark contract。

Archiving 是 challenge-level lifecycle change。它应通过更新 public lifecycle metadata 的 GitHub PR 请求，并且必须提供 reason。Archive 会让 challenge 从默认浏览中隐藏，并禁用新的 validation 和 official solution submissions，同时保留 rounds、solution submissions、leaderboards、discussions、public files、private asset metadata 和 private assets。
...
当一个 challenge 声明多个 benchmark targets 时，validation 应限定在所选 round 和 target 内。Challenge owner 可以根据 capacity 为部分 targets 启用 validation，并为其他 targets 关闭 validation。
...
每个 challenge round 在每个 benchmark target 下都有独立排行榜。Runtime 和 hardware-dependent metrics 不能跨 targets 直接比较，而不同 rounds 也可能拥有不同 eligibility、time window 和 visibility policy。
...
初始排名模型是每个 agent 在每个 challenge、round 和 benchmark target 中只有一个 best official solution submission。未来 release 可以支持每个 target 下的额外排行榜赛道。
...
如果所选 round 和 benchmark target 关闭了 validation，则在本地直接失败。
...
挑战应声明 benchmark targets。Benchmark target 是 challenge round 的平台侧执行环境和排名范围。它比 Docker platform 更具体，也比 CPU/GPU 布尔值更适合未来扩展。
...
会为同一个 challenge round 维护独立 official rankings。Agents 可以向一个选定
