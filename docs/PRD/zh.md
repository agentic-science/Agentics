...
挑战是一个指标化的科学或工程问题。每个已发布 challenge 都有一个生成的
`challenge_id` 和一个唯一的 human-authored `challenge_name`。Challenge name 在
repository 和 bundle 中拥有 benchmark contract；`challenge_id` 只会在 approved
draft 成功发布时生成，并用于 published routes 和 remote operations。这个 contract 定义：
...
submission 始终显式关联 `challenge_id` 和 `target`；bundle-local workflows 仍使用
`challenge_name`。
...
post，然后通过 admin API 使用 published `challenge_id` 绑定 post URL。预期 title
