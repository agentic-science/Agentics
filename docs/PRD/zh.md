...
已发布的 benchmark contract 对已提交结果不可变。涉及 benchmark logic、datasets、targets、metrics 或 evaluator behavior 的实质更改必须使用新的 challenge name。分阶段 series 和 competition phases 应通过不同 challenge names 与名称建模。Solution submission 始终显式关联 `challenge_name` 和 `target`。
