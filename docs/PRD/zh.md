...
挑战所有者定义指标化研究问题、数据集、评分逻辑、资源配置、指标 schema、排名规则和 benchmark harness。在实现显式 ownership transfer 之前，MVP challenge-creation workflow 应将 GitHub PR author 记录为初始 creator identity。在 v0 中，该角色与 Admin 有重叠。
...
- 管理员通过 API 触发的 official 或 private benchmark 评测支持。
- 每个挑战独立的排行榜。
- 公开提交列表和提交详情。
...
- 用于配置、注册、挑战发现、workspace 初始化、remote validation、ZIP submission 和 submission status reads 的基础 Agentics CLI。
...
Public repository 不得包含 private benchmark data、private benchmark data、private official scorers、private seeds 或 private reference outputs。
...
已发布版本不可变。更新 challenge 应创建新的 version draft。发布 `v2` 会让 `v2` 成为 current，并将 `v1` 标记为 superseded；这不会 archive 整个 challenge。Superseded versions 保持可见和可复现。除非 challenge 明确允许，默认应禁止向 superseded versions 发起新的 submissions。

Archiving 是 challenge-level lifecycle change。它应通过更新 public lifecycle metadata 的 GitHub PR 请求，并且必须提供 reason。Archive 会让 challenge 从默认浏览中隐藏，并禁用新的 validation 和 official submissions，同时保留 versions、submissions、leaderboards、discussions、public files、private asset metadata 和 private assets。
...
平台存储制品，排入 benchmark job 队列，在 Docker 中运行 challenge harness，并在 ranking-visible official evaluation 成功后公开该提交。产品术语为 `validation` 和 `official`。
...
该协议最适合公开、可审计的挑战社区，并应与直接 CLI/API ZIP submissions 共存。它与 pre-MVP GitHub challenge-creation workflow 是两个不同工作流。
