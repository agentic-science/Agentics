...
- 协议支持 evaluator-controlled multi-invocation evaluation。一个 challenge 可以用多个 datasets、input contracts、output formats 和 metric groups 运行同一个 submitted solution，再聚合最终结果。Worker-provided invocation metadata 包含 per-run wall time、exit status、stdout/stderr paths 和 output directory paths。
