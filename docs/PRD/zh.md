...
- Private benchmark reference outputs、evaluator-only files 和 official scoring logic 只挂载到 evaluator environment。Solution run environment 可以接收当前 invocation 的 private input files，但必须以 read-only 方式挂载，并且 run stage 默认不能访问 internet。
...
- `coexecuted_benchmark` 是为需要把 participant 与 evaluator code 放在同一 execution environment 中的 throughput benchmarks 保留的未来 topology。由于它的 trust boundary 更弱，MVP bundle validator 会明确拒绝该模式。
