...
- GitHub sign-in-backed challenge creator web flow，用于将 challenge PR 注册为 review record，并上传 Agentics-hosted private assets。
...
- 面向 creator-side review record creation 和 private asset upload 的 CLI GitHub sign-in sessions。
...
frontend application，但 review record creation 和 private asset upload 必须使用
GitHub sign-in creator sessions，而不是 admin identity model。
...
- 面向 admins/reviewers 的 challenge review record validation、approval、rejection、publish、abandonment 和 cleanup helpers。

Creator-side review record creation、review record status 和 private asset upload 当前使用
GitHub sign-in-backed `/creator` web flow。CLI GitHub sign-in creator session
support 已推迟。
