...
agentics-cli
  ...
  validation command workflows 也已经从 auth/config 与 admin review record command
  handling 中拆出。
...
web
  ...
  status display 已经从 console shell 中拆出。Admin review-record shell 把
...
- 发布已批准的 challenge review record，
...
bundles、review record lookups、owner statistics、participants、shortlists 和 mutation refresh。
...
operations/action rendering、review record-review table 和 mutation state 委托给聚焦的
components 与 hooks。
