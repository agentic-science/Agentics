...
`AGENTICS_WEB_PORT` 和 `AGENTICS_WEB_HOST_PORT` 会让 startup 失败，而不是回退到
local defaults。Dev launcher 也会拒绝使用 production 或 rehearsal 的 API/web host
ports（`3100`、`3001`、`13100` 和 `13001`）。
启用 host probing 时，`AGENTICS_HOST_PROBE_COMMAND` 必须是非空值。
...
1. 启动 Compose dev stack：
   ...
   Dev、test、rehearsal 和 production Compose 运行 PostgreSQL 18，image 为
   `postgres:18-alpine`，Postgres data 挂载到 `/var/lib/postgresql`，并设置
   `io_method=io_uring`。Postgres service 使用 `seccomp=unconfined`，因为当前
   Docker 默认 seccomp profile 会阻止 PG 18 `io_uring`。
...
3. 构建并启动：
   ...
   `just prod::up` 会在 Docker 创建默认 network 后检查 production Compose bridge forwarding rules，然后检查 dedicated runner Docker bridge forwarding rules。在 Linux host 上，它会安装幂等的 `DOCKER-USER` rules，允许 Compose 和 runner bridges 访问 host 的默认 outbound interface，并允许 established return traffic。这样 GitHub sign-in、API egress，以及需要网络的 evaluator setup phases 都不会依赖已经过期的 Docker forwarding state。
...
5. Production 使用与 dev、test、rehearsal 相同的 PostgreSQL 18 Compose 路径：
   `postgres:18-alpine`，data 挂载到 `/var/lib/postgresql`，并以 Postgres-only
   `seccomp=unconfined` 运行 `io_method=io_uring`。当前 production active volume 是
   `agentics-prod_postgres_data_pg18`。

   在约定 rollback window 结束前，请保留旧 PG16 volume
   `agentics-prod_postgres_data` 和 cold backup archive。不要对 production 运行
   `down -v`。Rollback 依赖 snapshot 或 volume，不依赖手动 down migration。

   Production restart 后可以这样验证 database：

   ```bash
   docker compose --env-file deploy/compose/env/prod.env \
...
production 或 rehearsal purge 删除 backup copy。当 production 或 production
rehearsal 启动自己的 RustFS 或 S3 bucket 后，需要先把所需 private bundle objects
从这个 backup store 复制到对应 environment 的 storage，再复用已经 migrated 的
challenge metadata：

```bash
just prod::restore-private-bundles
# 或者，对于 disposable rehearsal stack：
just rehearsal::restore-private-bundles --overwrite
```

restore command 会临时把 backup RustFS container 加入所选 Compose network，然后运行
一个一次性 service；该 service 可以访问两个 private RustFS endpoints。它会把
objects 复制到该 environment bucket 的 `AGENTICS_S3_PREFIX` 下，并放在逻辑
...
refresh window 中，才使用 `--overwrite` 来替换 destination 中已有但内容不同的
objects。`just storage::backup-down` 会停止 backup container，但不会删除 objects。
...
Production check 会验证同一组 Compose 和 runner bridge forwarding rules，从 API container 探测到 GitHub 的 HTTPS egress，并从 runner-container bridge 探测到 PyPI 的 HTTPS egress。GitHub egress 支撑 browser sign-in，runner egress 支撑需要网络的 setup phases，例如 evaluator dependency installation。
