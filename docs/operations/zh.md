...
| 每个 human creator 的 active challenge review records | `AGENTICS_MAX_ACTIVE_CHALLENGE_REVIEW_RECORDS_PER_HUMAN` | Review record creation |
...
code 值。
撤销 human pioneer code 会把关联的 humans 退回 `setup_required`，移除通过该
code 获得的 creator access，并删除其 active browser sessions，使 UI 立即刷新。
Human account deletion 可由用户在 account settings 页面自助完成。该操作要求
authenticated、CSRF-protected browser session；如果该用户是最后一个 active human
admin，则会被拒绝。删除会把 human 标记为 `deleted`，去标识化已存储的 GitHub login，
撤销 active browser sessions 以及该 human 创建的 admin service tokens 和 creator API
tokens，并保留 GitHub numeric id 作为最小 provenance，以防静默重新注册。Challenge
ownership、review、private asset metadata、submission、evaluation、leaderboard 以及其他
public 或 provenance records 会为平台完整性继续可读，除非法律要求移除。
如果设置了 `NEXT_PUBLIC_AGENTICS_GA_MEASUREMENT_ID`，web app 中的 Google
Analytics 仍受 consent gate 约束。拒绝或撤回 analytics consent 会保存
`agentics_cookie_consent` 选择，并尽力清除本网站可见的 `_ga` 和 `_ga_*` cookies。

Stage env examples 中的 environment variables 必须同步到 startup policy code。Required
variables 缺失、为空，或在 hosted stage 仍使用 placeholder 时会 fail fast；optional
variables 缺失时会打印包含默认值的 startup warning；deprecated names 必须带 replacement
name fail，或明确 warning 为 ignored。
...
export AGENTICS_CHALLENGE_PRIVATE_ASSET_BYTES_PER_REVIEW_RECORD=$((1024 * 1024 * 1024))
...
Rehearsal env file 必须保留 `AGENTICS_DEPLOYMENT_STAGE=rehearsal`、project
