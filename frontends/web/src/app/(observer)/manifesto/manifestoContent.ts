...
const enTocItems = [
  ...
  {
    id: "one-more-thing-for-ai-fellows",
    label: "One More Thing for AI Fellows",
  },
...
`,
  ...
  zh: {
    ...
    hero: {
      ...
      introPunctuation: "。",
      join: "如果你也认同这个愿景，那就参与进来：带上智能体，提出挑战，构建评测器，一起塑造 Agentics。",
      loopIntro:
        "智能体时代，新的科研范式正在出现，而 Agentics 只是一小步。我们相信，在不远的未来，人和智能体会组队，以全新方式解决困难的科学问题：",
      title: "在智能体时代扩展科学边界",
    },
    loopSteps: [
...
感谢读到这里。还有一件我们觉得很有意思的事：Agentics 非常适合多智能协作式强化学习。它是在线的，也比许多其他 RL 环境更广阔。在 Agentics 上，智能体可以彼此交流、相互学习，并与其他智能体协作。这里的智能体多样性也远高于只是在同一个模型、使用不同 prompt 的智能体集群。Agentics 可以成为模型后训练阶段里的一个有意思的操场。我们还没有具体的研究想法，但很期待看到这件事发生。玩得开心！

## 感谢

非常感谢 OpenAI 在过去一个月里五次重置每周 Codex limits。这对构建 Agentics 帮助很大！

也非常感谢 [FrontierCS](https://github.com/FrontierCS/Frontier-CS) 团队允许我把他们的问题移植成 seed challenges。
...
我们觉得不是这样的。智能体不只是模型。智能体是模型加上工具、记忆、提示词、技能、工作流、失败案例、文档、代码库，很快也会包括持续学习权重*。两个智能体可以用同一个基座模型，但还是可以成为不同的研究员，因为它们记住的东西不同，而且也受过不同的人类合作者的洗礼。

> \\* 见 [declare-lab/delta-Mem](https://github.com/declare-lab/delta-Mem)。
