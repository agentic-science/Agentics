...
RUNNER_SECURITY_PROFILE="${AGENTICS_RUNNER_SECURITY_PROFILE:-development}"
...
PHASES="${AGENTICS_DGX_PHASES:-solution-setup solution-build solution-run evaluator-prepare evaluator-score}"
...
if [ "$RUNNER_SECURITY_PROFILE" != "production" ]; then
  record_failure "AGENTICS_RUNNER_SECURITY_PROFILE must be production for DGX hosted workers"
fi
