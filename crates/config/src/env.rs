...
impl TryFrom<RawAppEnv> for Config {
    ...
    fn try_from(raw: RawAppEnv) -> anyhow::Result<Self> {
        ...
        config.database_url = match raw.database.database_url {
            Some(value) => {
        ...
        config.log_level = string_or_default(
            "AGENTICS_LOG_LEVEL",
...
fn apply_storage_env(config: &mut Config, raw: RawStorageEnv) -> anyhow::Result<()> {
    config.storage_root = string_or_default(
        ENV_AGENTICS_STORAGE_ROOT,
        raw.storage_root,
        DEFAULT_STORAGE_ROOT,
    )?;
    config.storage_backend = raw.storage_backend.unwrap_or(DEFAULT_STORAGE_BACKEND);
    config.storage_work_root = optional_non_empty_string(raw.storage_work_root);
    config.s3_bucket = raw
        .s3_bucket
        .map(trimmed_string)
        .or_else(|| Some(DEFAULT_S3_BUCKET.to_string()));
    config.s3_prefix = optional_non_empty_string(raw.s3_prefix);
    config.s3_region = string_or_default(ENV_AGENTICS_S3_REGION, raw.s3_region, DEFAULT_S3_REGION)?;
    config.s3_endpoint_url = match raw.s3_endpoint_url {
...
fn apply_auth_env(config: &mut Config, raw: RawAuthEnv) -> anyhow::Result<()> {
    config.admin_username = string_or_default(
        ENV_AGENTICS_ADMIN_USERNAME,
        raw.admin_username,
        DEFAULT_ADMIN_USERNAME,
    )?;
    config.admin_password = match raw.admin_password {
        Some(value) => {
...
fn apply_moltbook_env(config: &mut Config, raw: RawMoltbookEnv) -> anyhow::Result<()> {
    config.moltbook_submolt_name = match raw.moltbook_submolt_name {
    ...
    config.moltbook_submolt_url = match raw.moltbook_submolt_url {
...
fn apply_oauth_env(config: &mut Config, raw: RawGithubOauthEnv) -> anyhow::Result<()> {
    config.github_oauth_client_id = optional_non_empty_string(raw.github_oauth_client_id);
    config.github_oauth_client_secret =
        optional_non_empty_string(raw.github_oauth_client_secret).map(SecretString::from);
    config.github_oauth_redirect_url = raw
        ...
        .transpose()?;
    config.github_oauth_authorize_url = match raw.github_oauth_authorize_url {
    ...
    config.github_oauth_token_url = match raw.github_oauth_token_url {
    ...
    config.github_api_user_url = match raw.github_api_user_url {
...
fn apply_runner_env(config: &mut Config, raw: RawRunnerEnv) -> anyhow::Result<()> {
    config.docker_host = optional_non_empty_string(raw.docker_host);
    config.host_probe_mode = raw.host_probe_mode.unwrap_or(DEFAULT_HOST_PROBE_MODE);
    config.host_probe_command = string_or_default(
        ENV_AGENTICS_HOST_PROBE_COMMAND,
        raw.host_probe_command,
        DEFAULT_HOST_PROBE_COMMAND,
    )?;
    config.runner_security_profile = raw
        .runner_security_profile
        .unwrap_or(DEFAULT_RUNNER_SECURITY_PROFILE);
    config.require_digest_pinned_images = raw
        .require_digest_pinned_images
        .unwrap_or(DEFAULT_REQUIRE_DIGEST_PINNED_IMAGES);
    config.runner_writable_storage_mode = raw
        .runner_writable_storage_mode
        .unwrap_or(DEFAULT_RUNNER_WRITABLE_STORAGE_MODE);
    config.runner_namespace = match raw.runner_namespace {
